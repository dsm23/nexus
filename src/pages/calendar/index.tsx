  ...
  const renderCalendarGrid = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        ...
        >
          <div
            className={`mb-1 text-sm font-medium ${isCurrentDay ? "flex size-6 items-center justify-center rounded-full bg-primary text-primary-foreground" : ""}`}
          >
            {day}
          </div>
          <div className="space-y-1">
            {dayEvents.slice(0, 3).map((event) => (
              <div
                key={event.id}
                className={`truncate rounded-sm p-1 text-xs ${getEventBgColor(event.type)} border`}
