...
const getFileIcon = (type: string) => {
  switch (type) {
    ...
    case "xls":
      return <File className="size-4 text-green-600" />;
    case "video":
    ...
    <CardContent className="p-4">
      <div className="space-y-3">
        <div className="flex items-start justify-between">
          ...
          >
            <Heart
              className={`size-4 ${resource.isFavorite ? "fill-red-500 text-red-500" : "text-gray-400"}`}
            />
          </Button>
        </div>
          ...
          <PageSection index={1}>
            <TabsList className="grid w-full max-w-md grid-cols-2">
              <TabsTrigger value="browse" className="gap-2">
                <Search className="hidden size-4 sm:inline" />
                Browse Resources
              </TabsTrigger>
              <TabsTrigger value="categories" className="gap-2">
                <BookOpen className="hidden size-4 sm:inline" />
                Categories
              </TabsTrigger>
            </TabsList>
          </PageSection>
